...
if (!authHeader) throw new Error("AUTH_HEADER environment variable is required");
if (!authHeaderValue) throw new Error("AUTH_HEADER_VALUE environment variable is required");

// Helper to create fetch handler for cron jobs
const createCronHandler = (endpoint, label) =>
  async function () {
...
// Job configurations
const jobConfigs = [
  { name: "5min", cronTime: isDev ? "*/10 * * * * *" : "*/5 * * * *", endpoint: "5min", label: "5MIN CRON" },
  { name: "30min", cronTime: isDev ? "*/10 * * * * *" : "*/30 * * * *", endpoint: "30min", label: "30MIN CRON" },
  { name: "2hour", cronTime: isDev ? "*/10 * * * * *" : "0 */2 * * *", endpoint: "2hour", label: "2HOUR CRON" },
  {
    name: "daily",
    cronTime: isDev ? "*/10 * * * * *" : "0 10 * * *",
    endpoint: "daily",
    label: "DAILY CRON @ 10AM ET",
  },
];
